...
import { CoreApiError, createPulsePostReply } from "../../../../../../lib/core-api";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const reply = await createPulsePostReply({
      postId,
      authorUserId,
      body: replyBody,
    });
    ...
    return socialActionError(error, "PULSE_POST_REPLY_FAILED", "Core API did not accept the reply.");
  }
}
...
function validationError(message: string) {
  return NextResponse.json(
    { ok: false, error: { code: "INVALID_PULSE_POST_REPLY", message } },
    { status: 422 },
  );
}
