...
- Vault transaction flow uses `/api/contracts/margin-intents/prepare` to fetch a real core API call payload, encodes it with `viem`, sends it through the Farcaster EVM wallet provider, and records the submitted transaction hash through `/api/contracts/transactions/:transactionId`.
